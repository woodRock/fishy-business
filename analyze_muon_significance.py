...
def run_analysis(file_path):
    ...
    for ds in datasets:
        ...
        data = {
            k: v["val_balanced_accuracy"].dropna().values
        ...
        print(f"{'='*90}")
        print(
            f"{'Configuration':<25} | {'N':<3} | {'Mean':<8} | {'Std':<8} | {'p (vs Muon)':<12} | {'Effect (d)':<10}"
        )
        print(f"{'-'*90}")
        ...
        for name, values in data.items():
            ...
            if name == "AugFormer Muon SOTA":
                ...
                u_stat, p_val = stats.mannwhitneyu(
                    muon_data, values, alternative="two-sided"
                )
    ...
    print(f"\n{'='*90}")
    print(
        "Note: * indicates statistically significant difference (p < 0.05) vs Muon SOTA."
    )
    print(f"{'='*90}\n")
