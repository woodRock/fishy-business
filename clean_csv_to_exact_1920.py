...
def clean_to_1920(file_path="wanb_export_csv.csv"):
    ...
    df_clean = df[
        df["tmp_ds"].isin(expected_datasets) & df["tmp_mod"].isin(expected_models)
    ].copy()
    ...
    for ds in expected_datasets:
        for model in expected_models:
            subset = df_clean[
                (df_clean["tmp_ds"] == ds) & (df_clean["tmp_mod"] == model)
            ]
            if len(subset) > 30:
    ...
    if len(df_final) == 1920:
        ...
        print(
            f"⚠️  Warning: Final count is {len(df_final)}, not 1920. Not overwriting yet."
        )
