...
def flatten_dataloader(data_loader: torch.utils.data.DataLoader) -> List[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]:
    data = []
...
def get_embeddings(individual: list, data: list) -> Tuple[np.ndarray, np.ndarray, list]:
    ...
    for sample1, sample2, label_tensor in data:
        ...
        outputs1.append(e1); outputs2.append(e2)
...
def calculate_accuracy(individual: list, data: list) -> float:
    ...
    if np.std(outputs1) < 1e-6 or np.std(outputs2) < 1e-6: return 0.5
    ...
    scores = [balanced_accuracy_score(true_labels, sims > t) for t in np.linspace(0, 1, 50)]
    return max(scores)
...
def calculate_loss(individual: list, data: list) -> tuple[float]:
    try:
        ...
        for sample1, sample2, label_tensor in eval_data:
            ...
            emb1 = torch.tensor([gene(*s1_np) for gene in individual], dtype=torch.float32)
            emb2 = torch.tensor([gene(*s2_np) for gene in individual], dtype=torch.float32)
            
            emb1, emb2 = torch.nan_to_num(emb1), torch.nan_to_num(emb2)

            if torch.std(emb1) < 1e-6 or torch.std(emb2) < 1e-6: return (float('inf'),)
...
def eval_wrapper_for_map(individual: list, data: list, pset: gp.PrimitiveSet) -> tuple[float]:
...
def main():
    ...
    train_data, val_data = flatten_dataloader(train_loader), flatten_dataloader(val_loader)
    print(f"Data ready. Train pairs: {len(train_data)}, Validation pairs: {len(val_data)}\n")
    ...
    toolbox.register("individual", tools.initRepeat, creator.Individual, toolbox.tree, n=N_OUTPUTS)
    ...
    with multiprocessing.Pool() as pool:
        ...
        for gen in range(N_GENS):
            ...
            fitnesses = toolbox.map(functools.partial(evaluate_for_pool, data=eval_data_subset), pop)
            ...
            compiled_best = [gp.compile(expr=gene, pset=pset) for gene in current_best_ind]
            ...
            print(f"Gen {gen:02d}: Min Loss={min_loss:.4f} | Train Acc={train_acc:.4f} | Val Acc={val_acc:.4f}")
