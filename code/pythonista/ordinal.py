...
class TransformerOrdinal(nn.Module):
    def __init__(self, input_features, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, method='regression', num_classes=None, batch_first=True):
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model, nhead, dim_feedforward, dropout, batch_first=batch_first)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_encoder_layers)
        self.d_model = d_model
        
        if method == 'regression': self.output_layer = nn.Linear(d_model, 1)
        elif method == 'classification': self.output_layer = nn.Linear(d_model, num_classes)
        elif method == 'coral': self.output_layer = nn.Linear(d_model, num_classes - 1)
        elif method == 'clm':
    ...
    def forward(self, src):
        ...
        if self.method == 'clm':
            ...
            cutpoints = torch.cumsum(torch.nn.functional.softplus(self.cutpoints_gaps), dim=0)
...
class CumulativeLinkLoss(nn.Module):
    def forward(self, cum_logits, y_true):
        ...
        padded_cum_probs = torch.cat([torch.zeros_like(cum_probs[:, :1]), cum_probs, torch.ones_like(cum_probs[:, :1])], dim=1)
...
def label_to_coral_levels(labels, num_classes): return torch.stack([torch.where(labels >= i, 1.0, 0.0) for i in range(1, num_classes)], dim=1)
def coral_logits_to_prediction(logits): return torch.sum(torch.sigmoid(logits) > 0.5, dim=1)
def read_reims_excel_file(fp): return pd.read_excel(fp)
def filter_data_for_oil(df): return df[df['m/z'].str.contains('MO ', na=False)].copy()
def remove_first_two_characters(df): return df['m/z'].str[2:]
...
def get_dataloader(df, batch_size=32):
    ...
    for start in range(0, len(df), batch_size): yield df.iloc[start:min(start + batch_size, len(df))]

# -------------------------------------------------------------------
# --- 4. Training and Evaluation Functions
# -------------------------------------------------------------------
def train(model, optimizer, criterion, train_df, val_df, epochs, device, batch_size, method, num_classes, patience, checkpoint_path, verbose=False):
    early_stopper = EarlyStopping(patience=patience, verbose=verbose, path=checkpoint_path)
    ...
    for epoch in range(epochs):
        ...
        for batch in get_dataloader(train_df, batch_size):
            ...
            y_true_labels = torch.tensor(y.astype(np.int64), dtype=torch.long).to(device)
            
            if method == 'regression': y_target = y_true_labels.float().view(-1, 1)
            elif method in ['classification', 'clm']: y_target = y_true_labels
            elif method == 'coral': y_target = label_to_coral_levels(y_true_labels, num_classes).to(device)
        ...
        val_mae, val_bca = evaluate(model, get_dataloader(val_df, batch_size), device, method)
        if verbose: print(f"Epoch {epoch+1:02d}/{epochs}, Val MAE: {val_mae:.4f}, Val BCA: {val_bca:.4f}")
...
def evaluate(model, data_loader, device, method):
    ...
    with torch.no_grad():
        for batch in data_loader:
            x, y = batch.drop(columns=['m/z']).values, batch['m/z'].values
            x_tensor = torch.tensor(x, dtype=torch.float32).unsqueeze(-1).to(device)
            y_tensor_int = torch.tensor(y.astype(np.int64)).to(device) # True labels as int
            ...
            if method == 'regression':
                ...
                preds_int = torch.round(preds_float).long() # Round to nearest int for class
    ...
    try:
        ...
        print(f"Warning: Could not compute BCA for method '{method}'. May be missing classes in predictions.")
        bca = 0.0
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available(): device = "cuda"; print("🚀 CUDA is available! Using GPU.")
    elif torch.backends.mps.is_available(): device = "mps"; print("🚀 MPS is available! Using Apple Silicon GPU.")
    else: device = "cpu"; print("🐌 No GPU available, using CPU.")
    ...
    all_results = {method: {'Train MAE': [], 'Train BCA': [], 'Test MAE': [], 'Test BCA': []} for method in methods_to_compare}
    ...
    for fold, (train_val_index, test_index) in enumerate(skf.split(X, y_skf)):
        ...
        for method in methods_to_compare:
            ...
            model = TransformerOrdinal(
                input_features=1, d_model=D_MODEL, nhead=N_HEAD, num_encoder_layers=NUM_LAYERS,
                dim_feedforward=DIM_FEEDFORWARD, dropout=0.2, method=method, num_classes=NUM_CLASSES
            ).to(device)
            
            optimizer = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=1e-4)
            
            if method == 'regression': criterion = nn.MSELoss()
            elif method == 'classification': criterion = nn.CrossEntropyLoss()
            elif method == 'coral': criterion = nn.BCEWithLogitsLoss()
            elif method == 'clm': criterion = CumulativeLinkLoss()

            train(model, optimizer, criterion, train_df, val_df, EPOCHS, device, 
                  BATCH_SIZE, method, NUM_CLASSES, PATIENCE, checkpoint_path, verbose=False)
            
            model.load_state_dict(torch.load(checkpoint_path))
            
            train_mae, train_bca = evaluate(model, get_dataloader(train_df, BATCH_SIZE), device, method)
            test_mae, test_bca = evaluate(model, get_dataloader(test_df, BATCH_SIZE), device, method)
