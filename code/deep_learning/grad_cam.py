...
def analyze_with_gradcam(
    model, data_loader, device, output_dir="gradcam_results", num_samples=5
):
    """
    ...
    for batch_idx, (features, labels) in enumerate(data_loader):
        ...
        for i in range(min(len(features), num_samples - sample_count)):
            ...
            print(
                f"Saved visualization for sample {sample_count+1} ({is_correct} prediction)"
            )
            sample_count += 1
    ...
    for class_idx in sorted(class_total.keys()):
        ...
        print(
            f"Class {class_idx}: {accuracy:.2f}% ({class_correct[class_idx]}/{class_total[class_idx]})"
        )
    ...
    correct_cams = [
        ...
        if item["predicted_class"] == item["true_class"]
    ]
    if correct_cams:
