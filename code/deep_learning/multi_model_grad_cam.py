...
def load_model(model_class: nn.Module, model_path: os.path, device: torch.device, load_if_exists: bool = True, **kwargs) -> nn.Module:
...
def plot_top_features_comparison(feature_sets: List[List[int]], model_names: List[str], output_path: os.path) -> None:
    ...
    for i, (features, name) in enumerate(zip(feature_sets, model_names)):
        ...
        plt.scatter(features, y_values, label=name, alpha=0.7, s=50)  # Reduced size to 50
...
def main():
    ...
    for name, config in model_configs.items():
        ...
        try:
            model = load_model(config["class"], config["path"], device, load_if_exists=config.get("load_if_exists", True), **config["params"])
            
            # If the model was newly initialized (i.e., not loaded), train it
            if not os.path.exists(config["path"]) or not config.get("load_if_exists", True):
                ...
                model = train_model(
                    ...
                    val_loader=prepare_data_loaders(data_module.get_train_dataloader())[1], # Get validation loader
                    device=device,
