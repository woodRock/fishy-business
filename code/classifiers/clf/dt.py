...
import warnings
from sklearn.exceptions import ConvergenceWarning, FitFailedWarning # FitFailedWarning might still be relevant
...
# --- Configuration ---
target_threshold = 0.7143 # This threshold can be adjusted based on expectations for Decision Trees
random_seed = 42
# ---------------------

# Suppress warnings
warnings.filterwarnings("ignore", category=ConvergenceWarning) # Less relevant for DT, but harmless
...
def plot_feature_importances(importances, feature_names, title, dataset_name, top_n=30):
    ...
    if len(importances) != len(feature_names):
        print(f"Error: Mismatch in number of importances ({len(importances)}) and feature names ({len(feature_names)}).")
    ...
    sns.barplot(x='importance', y='feature', data=plot_df, palette="viridis_r") # Using a sequential palette
...
def main(dataset="species"):
    print(f"Starting Decision Tree Classifier GridSearchCV script for dataset: {dataset}...") # Updated
    ...
    param_grid = [
        {
            ...
            'classifier__ccp_alpha': [0.0, 0.001, 0.005, 0.01] # Cost-complexity pruning
        }
    ]
    ...
    try:
        ...
        for param_name_full in sorted(best_params.keys()):
            if param_name_full.startswith("classifier__"): # Print only classifier hyperparams
        ...
        print("\n--- Visualizing Feature Importances for Best Decision Tree Model ---") # Updated
        best_pipeline = grid_search.best_estimator_
        dt_model = best_pipeline.named_steps['classifier'] # Now a DecisionTreeClassifier
        ...
        try:
            ...
            print("Warning: `get_feature_names_out` not available on the preprocessor object.")
            print("This typically means an older scikit-learn version. Consider upgrading for full feature name support.")
        ...
        if feature_names:
            ...
            plot_feature_importances(importances, feature_names, plot_title, dataset_name=dataset)
        # -------------------------------------------------------------
        ...
        if "Cannot have number of splits n_splits=" in str(ve) or "The least populated class" in str(ve):
            print(
