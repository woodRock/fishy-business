...
class nb(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        for i, c in enumerate(self.classes_):
            ...
            # Weighted mean
            self.theta_[i, :] = np.average(
                X_c, weights=np.full(X_c.shape[0], weight_c), axis=0
            )

            # Weighted variance
            diff_sq = (X_c - self.theta_[i, :]) ** 2
            self.sigma_[i, :] = np.average(
                diff_sq, weights=np.full(X_c.shape[0], weight_c), axis=0
            )
    ...
    def _joint_log_likelihood(self, X):
        ...
        for i in range(len(self.classes_)):
            ...
            joint_log_likelihood.append(
                np.sum(log_probs, axis=1) + np.log(self.class_priors_[i])
            )
