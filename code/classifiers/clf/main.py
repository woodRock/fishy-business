...
if __name__ == "__main__":
    ...
    for dataset, classifiers in results.items():
        ...
        for classifier, metrics in classifiers.items():
            ...
            print(
                f"    F1-score: {metrics['f1-score']:.2f}\% $\pm$ {metrics['f1-score_std']:.2f}\%"
            )
            print(
                f"    Precision: {metrics['preision']:.2f}\% $\pm$ {metrics['precision_std']:.2f}\%"
            )
            print(
                f"    Recall: {metrics['recall']:.2f}\% $\pm$ {metrics['recall_std']:.2f}\%"
            )
