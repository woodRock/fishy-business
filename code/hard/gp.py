...
def parse_arguments():
    ...
    parser.add_argument('-nt', '--num-trees', type=int, default=24,
...
# Evaluation function
def evalBalancedAccuracyLoss(individual: List[gp.PrimitiveTree], data: List[Tuple[np.ndarray, np.ndarray, float]]) -> Tuple[float]:
    ...
    for x,y in data:
        ...
        preds = torch.tensor(np.argmax(np.array([tree(*x) for tree in trees])), dtype=torch.float32)
...
def extract_features(individual: List[gp.PrimitiveTree], data: List[Tuple[np.ndarray, np.ndarray, float]]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for x,y in data:
        output_x1 = np.array([tree(*x) for tree in trees])
        features_x1.append(output_x1)
        labels.append(y)
    
    return np.array(features_x1), np.array(labels)

def plot_features(features_x1: np.ndarray, labels: np.ndarray):
    # Use PCA to reduce dimensionality to 2D for visualization
    pca = PCA(n_components=2)
    features = pca.fit_transform(features_x1)

    plt.figure(figsize=(12, 6))
    
    # Plot x1 features
    plt.scatter(features[:, 0], features[:, 1], c=labels, cmap='tab20', alpha=0.7)
...
def main() -> Tuple[List[List[gp.PrimitiveTree]], tools.Logbook, tools.HallOfFame]:
    ...
    def loader_to_list(loader: torch.utils.data.DataLoader) -> List[Tuple[np.ndarray, np.ndarray, float]]:
        ...
        for x,y in loader:
            for i in range(len(y)):
                ...
                data_list.append((x[i].numpy(), y[i].item()))
        return data_list
    ...
    toolbox.register("evaluate", evalBalancedAccuracyLoss, data=train_data)
    ...
    best_fitness = evalBalancedAccuracyLoss(best_individual, val_data)
    ...
    features_x1, labels = extract_features(best_individual, train_data)
    plot_features(features_x1, labels)
