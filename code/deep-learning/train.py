...
def transfer_learning(
    ...
    if dataset not in output_dims:
        raise ValueError(f"Invalid dataset specified: {dataset}. Must be one of {list(output_dims.keys())}")
...
def _train_fold(
    ...
    for epoch in tqdm(range(num_epochs), desc="Training"):
        # Training phase
        model.train()
        train_results = _run_epoch(model, train_loader, criterion, optimizer, device, True)
        
        # Validation phase
        model.eval()
        with torch.no_grad():
            val_results = _run_epoch(model, val_loader, criterion, optimizer, device, False)
        ...
        if current_val_accuracy > best_val_accuracy:
            ...
            logger.info(f"Epoch {epoch + 1}: New best validation accuracy: {best_val_accuracy:.4f}")
...
def _run_epoch(
    ...
    for inputs, labels in loader:
        ...
        if (isinstance(model, VAE)):
            ...
            outputs = model(inputs, inputs) if isinstance(model, Transformer) else model(inputs)
