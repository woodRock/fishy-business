...
def perform_significance_test(model_results: List[float], baseline_results: List[float], alpha: float = 0.05) -> Dict[str, Any]:
    ...
    if len(m_res) != len(b_res):
        if len(m_res) > len(b_res):
            b_res = np.pad(b_res, (0, len(m_res) - len(b_res)), mode='constant', constant_values=np.mean(b_res))
        else:
            m_res = np.pad(m_res, (0, len(b_res) - len(m_res)), mode='constant', constant_values=np.mean(m_res))
...
def summarize_results(results_map: Dict[str, List[Dict[str, Any]]], baseline_model: Optional[str] = None):
    ...
    for dataset, models in datasets.items():
        ...
        logger.debug(f"Dataset {dataset}: Using {actual_baseline} as baseline. Available: {list(models.keys())}")
        ...
        for model_name, m_data in models.items():
            ...
            logger.debug(f"  - Testing {model_name} (n={len(m_val)}) against {actual_baseline} (n={len(b_val_accs)})")
            ...
            if model_name == actual_baseline:
                ...
                if sig_test == " ": logger.debug(f"    * No significance symbol for {model_name}. Reason: {sig_res.get('error', 'None')}")
