...
class AutomatedBiomarkerDiscovery:
    ...
    def query_lipidmaps(self, mz: float, tolerance: float = 0.1) -> List[Dict[str, Any]]:
        """
                ...
                if len(lines) > 1:
                    ...
                    for line in lines[1:4]: # Take top 3
                        ...
                        if len(parts) == len(header):
                            ...
                            results.append({
                                "name": row.get("name", row.get("common_name", "Unknown")),
    ...
    def identify_top_peaks(self, importance_scores: np.ndarray, top_n: int = 5) -> List[Dict[str, Any]]:
        """
    ...
    def generate_report(self, biomarker_results: List[Dict[str, Any]]) -> str:
        ...
        for res in biomarker_results:
            ...
            if res["matches"]:
                ...
                match_str = f"[bold green]{best_match['name']}[/] ({best_match['source']})"
            
            lines.append(f"Peak m/z {res['m_z']:.4f} (Imp: {res['importance']:.4f}) -> {match_str}")
