...
class RunContext:
    ...
    def __init__(
        self, experiment_name: str, run_id: int = 0, base_output_dir: str = "outputs"
    ):
        ...
        self.run_dir = (
            Path(base_output_dir) / experiment_name / f"run_{run_id}_{self.timestamp}"
        )
        ...
        self.logger.info(
            f"Initialized RunContext for experiment: {experiment_name}, run: {run_id}"
        )
