...
class DataProcessor:
    ...
    def encode_labels(self, data: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if enc_type == "sklearn":
            ...
            raise ValueError(f"Unknown label encoding type for {self.dataset_name}")
        if y.ndim == 1:
            y = y[:, np.newaxis]
        return X, y
...
class DataModule:
    ...
    def get_groups(self) -> Optional[np.ndarray]:
        if self.filtered_data is None:
            self.setup()
        return (
            self.processor.extract_groups(self.filtered_data)
