    ...
    def _get_model_instance(self) -> Any:
        ...
        if self.method == "evolutionary":
            ...
            if self.model_name in ["gp", "ga", "eda"]:
                return model_class(generations=gens, population_size=pop, random_state=self.run_id)
            elif self.model_name == "es":
                return model_class(generations=gens, mu=pop // 2, lambda_=pop, random_state=self.run_id)
            elif self.model_name == "pso":
                return model_class(iterations=gens, population_size=pop, random_state=self.run_id)
            else:
                return model_class()
        ...
        for fold, (train_idx, test_idx) in enumerate(skf.split(X_scaled, y), 1):
            ...
            if self.config.regression:
                ...
                if y_pred.dtype.kind in 'fc': # float or complex
                    self.logger.debug("Rounding continuous predictions for classification metrics.")
                    y_pred_metrics = np.round(y_pred).clip(min(y), max(y)).astype(int)
                    y_train_pred_metrics = np.round(y_train_pred).clip(min(y), max(y)).astype(int)
                else:
            ...
            metric_name = "epoch/train_r2" if self.config.regression else "epoch/train_balanced_accuracy"
            val_metric_name = "epoch/val_r2" if self.config.regression else "epoch/val_balanced_accuracy"
            ...
            if fold == self.config.k_folds:
                ...
                if self.ctx.wandb_run:
                    if self.config.regression:
                        ...
                        analyze_regression_predictions(last_fold_info, fold, self.ctx, dataset_name=self.dataset_name)
                    else:
                        ...
                        table_probs = y_probs if y_probs is not None else np.eye(len(class_names))[y_pred.astype(int)]
...
def run_sklearn_experiment(
    ...
    trainer = SklearnTrainer(config, model_name, dataset_name, run_id, file_path, wandb_run=wandb_run)
    try:
        return trainer.run()
    finally:
