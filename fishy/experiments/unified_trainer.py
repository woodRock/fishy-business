...
class UnifiedTrainer:
    ...
    def _run_single(self, config: TrainingConfig) -> Dict[str, Any]:
        ...
        ctx.logger.info(f"Starting {config.method} training for {config.model} on {config.dataset}")
    ...
    def _dispatch_sklearn(self, config: TrainingConfig, wandb_run: Any):
        ...
        return run_sklearn_experiment(
            config, config.model, config.dataset, config.run, config.file_path, wandb_run=wandb_run
        )
    ...
    def _do_benchmark(self, config: TrainingConfig, ctx: RunContext, device: torch.device, training_time: float):
        data_module = create_data_module(dataset_name=config.dataset, file_path=config.file_path)
        data_module.setup()
        input_dim = data_module.get_input_dim()
        run_benchmark(model=None, input_dim=input_dim, device=device, ctx=ctx, training_time=training_time)

    def _generate_figures(self, config: TrainingConfig, ctx: RunContext, results: Dict[str, Any]):
        ...
        if "epoch_metrics" in results:
            ...
            plt.title("Loss vs. Step"); plt.legend()
            ...
            plt.title("Accuracy vs. Step"); plt.legend()
            ctx.save_figure(plt, "training_curves.png")
            plt.close()

        preds_dict = results.get("best_val_predictions", results.get("predictions"))
        if preds_dict:
            y_true, y_pred, y_probs = preds_dict.get("labels"), preds_dict.get("preds"), preds_dict.get("probs")
