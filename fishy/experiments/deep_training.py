...
class ModelTrainer:
    ...
    def __init__(self, config: TrainingConfig, wandb_run: Optional[Any] = None, ctx: Optional[RunContext] = None):
        self.config = config
        ...
        dataset_name = "oil_regression" if (self.config.regression and self.config.dataset == "oil") else self.config.dataset
        self.data_module = create_data_module(file_path=config.file_path, dataset_name=dataset_name, batch_size=config.batch_size, augmentation_config=config)
        self.data_module.setup()
        self.n_features = self.data_module.get_input_dim()
        self.n_classes = self.data_module.get_num_classes()
        self.pre_train_orchestrator = PreTrainingOrchestrator(config=self.config, device=self.device, input_dim=self.n_features, ctx=self.ctx)
    ...
    def _train_single_split_siamese(self, pre_trained_model: Optional[nn.Module]) -> Tuple[nn.Module, Dict[str, Any]]:
        ...
        tr_val_idx, te_idx = train_test_split(idx, test_size=0.2, random_state=self.config.run, stratify=lbls)
        tr_idx, val_idx = train_test_split(tr_val_idx, test_size=0.25, random_state=self.config.run, stratify=lbls[tr_val_idx])
        tr_ldr = DataLoader(Subset(ds, tr_idx), batch_size=self.config.batch_size, shuffle=True)
        ...
        if pre_trained_model: self.pre_train_orchestrator.adapt_for_finetuning(model, pre_trained_model)
        ...
        trained_model, tr_val_met = DeepEngine.train_model(model=model, train_loader=tr_ldr, val_loader=val_ldr, criterion=criterion, optimizer=opt, num_epochs=self.config.epochs, patience=self.config.early_stopping, is_augmented=self.config.data_augmentation, device=self.device, num_classes=self.n_classes, regression=self.config.regression, ctx=self.ctx)
        test_res = DeepEngine.evaluate_model(trained_model, te_ldr, criterion, self.device, num_classes=self.n_classes, regression=self.config.regression)
        if self.ctx.wandb_run: self._log_advanced_visualizations(test_res, te_ldr)
        final_metrics = {"train_loss": tr_val_met.get("train_loss"), "val_loss": tr_val_met.get("val_loss"), "test_loss": test_res.get("loss"), "test_balanced_accuracy": test_res.get("metrics", {}).get("balanced_accuracy"), "predictions": test_res.get("predictions"), "epoch_metrics": tr_val_met.get("epoch_metrics")}
        self.ctx.save_results(final_metrics, filename="final_metrics.json")
        return trained_model, final_metrics

    def _train_kfold(self, pre_trained_model: Optional[nn.Module]) -> Tuple[nn.Module, Dict[str, Any]]:
        ...
        all_fold_metrics = []
        last_model = None
        for fold, (tr_idx, val_idx) in enumerate(skf.split(full_samples, np.argmax(full_labels, axis=1))):
            self.logger.info(f"--- Fold {fold + 1}/{k_folds} ---")
            tr_ldr = DataLoader(CustomDataset(full_samples[tr_idx], full_labels[tr_idx]), batch_size=self.config.batch_size, shuffle=True)
            val_ldr = DataLoader(CustomDataset(full_samples[val_idx], full_labels[val_idx]), batch_size=self.config.batch_size)
            model = create_model(self.config, self.n_features, self.n_classes).to(self.device)
            if pre_trained_model: self.pre_train_orchestrator.adapt_for_finetuning(model, pre_trained_model)
            criterion = nn.MSELoss() if self.config.regression else nn.CrossEntropyLoss(label_smoothing=self.config.label_smoothing)
    ...
    def _log_advanced_visualizations(self, results: Dict, loader: DataLoader):
        ...
        if not self.config.regression and y_probs is not None: self.ctx.log_summary_charts(y_true, y_probs, class_names)
