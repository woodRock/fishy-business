...
class Trainer:
    ...
    def train(
        ...
        for epoch in tqdm(
            ...
            epoch_log["learning_rates"].append(self.optimizer.param_groups[0]["lr"]) # Log LR
            ...
            if val_loader:
                ...
                if current_val_acc > best_val_accuracy:
                    ...
                    epochs_no_improve = 0
                    self.logger.debug(f"E{epoch+1}: New best val_acc: {best_val_accuracy:.4f}")
                else:
                    epochs_no_improve += 1
        ...
        if not val_loader:
            # If no validation, just return the last state
            best_model_state_cpu = OrderedDict(
                (k, v.clone().cpu()) for k, v in self.model.state_dict().items()
            )
            best_fold_metrics = self._compile_best_metrics(
        ...
        # Fallback if no best metrics found (e.g. patience=0 or immediate break)
    ...
    def _unpack_batch(self, batch):
        if len(batch) == 3: # Siamese
             return (batch[0], batch[1]), batch[2]
        return batch[0], batch[1]

    def _to_device(self, inputs, labels):
        if isinstance(inputs, tuple):
            return (inputs[0].to(self.device), inputs[1].to(self.device)), labels.to(self.device)
        return inputs.to(self.device), labels.to(self.device)

    def _forward_pass(self, inputs):
        if isinstance(inputs, tuple):
             return self.model(inputs[0], inputs[1])
        # Special handling for VAE if needed, though general Trainer should be generic.
        # Assuming standard model for now.
        if hasattr(self.model, "decode") and hasattr(self.model, "reparameterize"): # VAE check
    ...
    def _calculate_metrics(self, y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
        if self.regression:
             ...
             metrics["balanced_accuracy"] = balanced_accuracy_score(y_true.astype(int), rounded_preds)
             return metrics
