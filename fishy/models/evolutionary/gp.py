...
def xmate(
    ind1: List[PrimitiveTree], ind2: List[PrimitiveTree]
) -> Tuple[List[PrimitiveTree], List[PrimitiveTree]]:
...
def xmut(
    individual: List[PrimitiveTree], expr: Any, pset: PrimitiveSetTyped
) -> Tuple[List[PrimitiveTree]]:
...
class GP(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        self.pset.addPrimitive(
            lambda l, r: np.divide(l, r, out=np.ones_like(l), where=r != 0),
            [float, float],
            float,
            name="/",
        )
        ...
        toolbox.decorate(
            "mate", staticLimit(key=lambda t: t.height, max_value=self.max_depth)
        )
        toolbox.decorate(
            "mutate", staticLimit(key=lambda t: t.height, max_value=self.max_depth)
        )
