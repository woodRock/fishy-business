...
class GA(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        def evaluate(ind):
            ...
            return (
                balanced_accuracy_score(y[split:], clf.predict(X_w[split:]))
                + 0.01 * (1.0 - np.mean(w)),
            )
        ...
        n_elites = max(1, int(self.elitism * self.population_size))
        ...
        for gen in range(self.generations):
            off = [
                toolbox.clone(ind)
                for ind in toolbox.select(pop, self.population_size - n_elites)
            ]
            ...
            for ind, fit in zip(
                [i for i in off if not i.fitness.valid],
                map(toolbox.evaluate, [i for i in off if not i.fitness.valid]),
            ):
                ind.fitness.values = fit
            pop[:] = [toolbox.clone(i) for i in tools.selBest(pop, n_elites)] + off
            hof.update(pop)
