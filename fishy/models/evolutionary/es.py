...
class ES(BaseEstimator, ClassifierMixin):
    """Scikit-learn compatible wrapper for Evolution Strategy."""

    def __init__(self, generations: int = 10, mu: int = 50, lambda_: int = 100, random_state: int = 42):
        self.generations = generations; self.mu = mu; self.lambda_ = lambda_; self.random_state = random_state; self.best_individual = None
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        toolbox.register("evaluate", evaluate); toolbox.register("mate", tools.cxBlend, alpha=0.5); toolbox.register("mutate", tools.mutGaussian, mu=0, sigma=0.1, indpb=0.2); toolbox.register("select", tools.selBest)
        pop = toolbox.population(n=self.mu); hof = tools.HallOfFame(1)
        algorithms.eaMuPlusLambda(pop, toolbox, mu=self.mu, lambda_=self.lambda_, cxpb=0.6, mutpb=0.3, ngen=self.generations, halloffame=hof, verbose=False)
