...
def test_cli_parsing():
    parser = setup_parser()
    # Test a few commands
    args = parser.parse_args(["train", "--model", "transformer", "--dataset", "species"])
...
def test_transfer_learning_flow(tmp_path):
    ...
    assert "val_balanced_acc" in results["finetune"]["species"] or "val_balanced_accuracy" in results["finetune"]["species"]
