...
def test_gmoe_initialization():
    input_dim = 128
    output_dim = 2
    model = SparselyGatedMoE(
        input_dim=input_dim, output_dim=output_dim, num_experts=4, k=1
    )
...
def test_gmoe_forward_pass():
    ...
    output_dim = 3
    model = SparselyGatedMoE(
        input_dim=input_dim, output_dim=output_dim, num_experts=4, k=2
    )
...
def test_gmoe_3d_input():
    ...
    output_dim = 2
    model = SparselyGatedMoE(
        input_dim=input_dim, output_dim=output_dim, num_experts=2, k=1
    )
