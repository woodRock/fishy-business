...
def test_spectral_augmentation():
    ...
    cfg = AugmentationConfig(enabled=True, num_augmentations=1, noise_enabled=True, noise_level=0.5)
...
def test_pretraining_orchestrator(tmp_path):
    ...
    if not os.path.exists(data_file): pytest.skip("Data file not found")
    ...
    from torch.utils.data import DataLoader, TensorDataset
    loader = DataLoader(TensorDataset(torch.randn(16, 2080), torch.randint(0, 2, (16,))), batch_size=8)
