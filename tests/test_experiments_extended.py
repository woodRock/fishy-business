...
def test_unified_trainer_single(tmp_path):
    ...
    with patch("fishy.experiments.unified_trainer.get_device", return_value=torch.device("cpu")), \
...
def test_deep_training_pipeline_flow(tmp_path):
    ...
    cfg = TrainingConfig(model="dense", dataset="species", file_path=data_file, epochs=1, k_folds=2)
    with patch("fishy.experiments.deep_training.get_device", return_value=torch.device("cpu")), \
         patch("fishy.engine.trainer.get_device", return_value=torch.device("cpu")):
