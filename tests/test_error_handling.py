...
def test_unified_trainer_batch_error_handling():
    ...
    with mock.patch.object(UnifiedTrainer, "_run_single", side_effect=side_effect):
        ...
        with (
            mock.patch(
                "fishy.experiments.unified_trainer.summarize_results"
            ) as mock_sum,
            ...
            mock_sum.return_value = pd.DataFrame(
                [{"model": "working_model", "accuracy": 0.8}]
            )
...
def test_unified_trainer_run_single_cleanup():
    ...
    with (
        ...
        with mock.patch.object(
            UnifiedTrainer, "_dispatch_deep", side_effect=ValueError("Test Error")
        ):
