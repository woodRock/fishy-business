...
Their synchronous movement was modelled as a function of each fish mantaining equal distance to its neighbours. 
...
Each fish updates its movement in equal parts towards $p_{best}$ and $g_{best}$, both multiplied by a stochasity factor.
The stochasity introduces randomness, which makes the fish overshoot their target about half the time. 
The overshooting has fundmanetal to the success of the model, it allows the school to explore uncharted waters (unknown regions of the problem domain). 
...
The fitness function balances the SVM classification accuracy and the numbers of features selected.
