...
\begin{abstract}
  ...
  % TODO [x] - the significance of automating the process. 
  
  In fish processing worldwide, 60\% of fish doesn't make it onto a plate. 
  ...
  Efficient use of fish biomass aided by real-world AI can help preserve fish for future generations to come.
...
% TODO [x] - Provide a "problem statement". 
% TODO [x]  Try to state the current techniques for this problem and their key limitations of them, which will form the motivation for this work.

% PROBLEM STATEMENT 

% STEP 1. General Worldview of the problem area. 
Worldwide, 60\% of treated fish biomass isn't used in food \cite{galanakis2019saving}. 
It would be an incredible waste of the Earth's limited resources to discard this. 
Instead, fish biomass can be repurposed for fish oils - for products such as omega-3 vitamin supplements - or fish meal, both of which can for used in animal feeds. 
Further processing can produce refined fish oil that is suitable for fish oil capsules that we buy in shops and pharmacies. 
Some fish species or parts may be richer in omega-3 fatty acids than others, these can be used to make high-value products. 
To identify valuable fish oils we need to classify the species and part of tissue samples. 
...
Bi et al. proposed a CNN model that performs GC-MS data fusion in food science \cite{bi2020gc}. 
...
Matushuin et al. proposed a stacking model for analysis of GC data \cite{matyushin2020gas}.
It stacked results of a 1DConv, 2DConve, Deep residual MLP and XGBoost. 
Their regression model identified the retention index.  
...
Demelza et al. \cite{robinson2020genetic} proposed a feature and latent variable selection method for regression models in horticulture.
...
% TODO [x] - The description of SVMs can be shorted to save space. 
...
RBF is the default kernel, other models have different kernels and introduce hyperparameters. 
The NuSVC has hyperparameter \emph{Nu}, this determines the number of support vectors \cite{scholkopf2000new}. 
This is a trainable parameter in the other SVM models. 

The kernel function shapes the support vectors in the hyperplane.
Each kernel can capture patterns of varying complexity. 
The original SVM had a linear kernel \cite{aizerman1964theoretical}, 
Later, non-linear kernels that employ the kernel trick were introduced \cite{boser1992training}.
Figure~\ref{fig:kernels} shows support vectors for each kernel on a 2D plane, this provides an intuition for each kernel.
...
% TODO [x] - describes the analysis of why LSVM performed/generalized much better than SVC and NuSVC. 
...
% TODO [ ] - could you describe an analysis on why linear kernel performed/generalised much better than others? Why sigmoid kernel does not work. 
...
The SVC and Nu-SVC models both default to an RBF kernel. 
Nu-SVC has a fixed parameter \emph{Nu} that determines the number of support vectors \cite{scholkopf2000new}. 
A fixed hyper-parameter could explain its improved performance over SVC. 
...
It simplifies the feature space, removing noise, and revealing the important features of the model output. 
