...
def run_comparison(file_path):
    ...
    print(f"{'='*95}")
    print(
        f"{'Dataset':<15} | {'Aug EMA Mean':<12} | {'GMLP Base Mean':<14} | {'p-value':<10} | {'Effect (d)':<10}"
    )
    print(f"{'-'*95}")
    ...
    for ds in datasets:
        ...
        gmlp_base = (
            ds_df[(ds_df["model"] == "gatedmlp")]["val_balanced_accuracy"]
            .dropna()
            .values
        )
        ...
        print(
            f"{ds:<15} | {np.mean(aug_ema):.4f}       | {np.mean(gmlp_base):.4f}         | {p_val:.4f}{sig} | {d:.4f}"
        )
