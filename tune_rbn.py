...
def objective(trial: optuna.Trial) -> float:
    ...
    binding_type = trial.suggest_categorical("binding_type", ["hadamard", "outer_product"])
    ...
    try:
        ...
        is_oom = any(s in error_msg for s in ["out of memory", "allocat", "cuda error: out of memory", "mps"])
        
        if is_oom:
            console.print(f"[yellow]Trial {trial.number} failed due to OOM: {hidden_dim=}, {num_layers=}, {top_k=}. Skipping...[/]")
...
def run_study(n_trials: int = 20):
    ...
    console.print(f"[bold green]Starting Optuna study for RBN on species dataset ({n_trials} trials)...[/]")
