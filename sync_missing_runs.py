...
def sync_missing(
    ...
    for ds, model, current in tqdm(missing_targets):
        ...
        filters = {"config.dataset": ds, "config.model": model, "state": "finished"}
            ...
            new_records.append(
                {
                    ...
                    "dataset": ds,
                    "model": model,
                    "train_balanced_accuracy": run.summary.get(
                        "train_balanced_accuracy", run.summary.get("train_accuracy", 0)
                    ),
                    "val_balanced_accuracy": run.summary.get(
    ...
    if unique_new_df.empty:
        print(
            "⚠️  Found runs on W&B, but they are already in your CSV (matching by 'Name')."
        )
        return
