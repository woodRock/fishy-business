...
DEEP_MODELS = [
    "lstm", "vae", "kan", "cnn", "mamba", "transformer",
...
def main():
    ...
    for dataset in DATASETS:
        for model_name in CLASSIC_MODELS:
            logger.info(f"Running classic experiment: Model={model_name}, Dataset={dataset}")
            try:
                run_classic_experiment(model_name=model_name, dataset_name=dataset)
                logger.info(f"Successfully completed classic experiment: Model={model_name}, Dataset={dataset}")
            except Exception as e:
                logger.error(f"Error running classic experiment Model={model_name}, Dataset={dataset}: {e}")

    logger.info("Starting all deep learning experiments...")
    for dataset in DATASETS:
        for model_name in DEEP_MODELS:
            logger.info(f"Running deep learning experiment: Model={model_name}, Dataset={dataset}")
            ...
            if model_name == "transformer_msm":
                ...
                logger.info(f"Configuring Transformer with Masked Spectra Modelling pre-training for Dataset={dataset}")

            try:
                run_training_pipeline(config=config)
                logger.info(f"Successfully completed deep learning experiment: Model={model_name}, Dataset={dataset}")
            except Exception as e:
                logger.error(f"Error running deep learning experiment Model={model_name}, Dataset={dataset}: {e}")
