...
def analyze_benchmarks(file_path):
    ...
    print(
        f"{'Dataset':<12} | {'Config':<15} | {'Mean Acc':<10} | {'Std Dev':<10} | {'p-val':<10} | {'Effect (d)':<10}"
    )
    print("-" * 85)
    ...
    for ds in datasets:
        ...
        baseline = (
            ds_df[(ds_df["use_ema"] == False) & (ds_df["use_ttt"] == False)][
        ...
        ema_only = (
            ds_df[(ds_df["use_ema"] == True) & (ds_df["use_ttt"] == False)][
                "val_balanced_accuracy"
            ]
            .dropna()
            .values
        ...
        for name, data in groups:
            ...
            if name == "Baseline":
                print(
                    f"{ds:<12} | {name:<15} | {mean:.4f}   | {std:.4f}   | {'N/A':<10} | {'N/A':<10}"
                )
            else:
                # Mann-Whitney U Test
                u_stat, p_val = stats.mannwhitneyu(
                    data, baseline, alternative="two-sided"
                )
                d = cohen_d(data, baseline)

                sig = "*" if p_val < 0.05 else " "
                print(
                    f"{ds:<12} | {name:<15} | {mean:.4f}   | {std:.4f}   | {p_val:.4f}{sig} | {d:.4f}"
                )
        print("-" * 85)
