...
def process_wandb_csv(file_path):
    ...
    try:
        ...
        if not all(
            c in df.columns for c in ["dataset", "model", "val_balanced_accuracy"]
        ):
        ...
        for (ds, model), group in df.groupby(["dataset", "model"]):
            if pd.isna(ds) or pd.isna(model) or ds == "dataset":
                continue
            key = f"{ds}|||{model}"
            runs = []
...
def render_advanced_benchmarks(df_summary, df_raw, color_map):
    ...
    tab_box, tab_heat, tab_radar = st.tabs(
        ["🛡️ Stability", "📈 Heatmap & Efficiency", "🏆 Top 3 Profiles"]
    )
    ...
    with tab_heat:
        ...
        with c_h1:
            ...
            pivot_df = df_summary.pivot(
                index="Method", columns="Dataset", values="Test"
            )
            fig_heat = px.imshow(
                pivot_df,
                text_auto=".3f",
    ...
    with tab_radar:
        ...
        for _, row in top_3.iterrows():
            ...
            f1_val = row.get(
                "f1", row.get("F1 Score", test_val)
            )  # fallback to test acc
...
if data_path.exists():
    ...
    feature_cols = [
        c for c in df_filtered.columns if c not in [label_col, "m/z", "Label"]
    ]
    # Fix: Ensure column names are treated as strings before calling .replace()
    mz_axis = (
        np.array([float(c) for c in feature_cols])
        if all(str(c).replace(".", "", 1).isdigit() for c in feature_cols)
        else np.arange(len(feature_cols))
    )
                        ...
                        if rep_idx is not None and c_idx in class_biomarkers:
                            ...
                            comp_fig.add_trace(
                                go.Scatter(
                                    ...
                                    y=X_xai[rep_idx][class_biomarkers[c_idx]],
                                    mode="markers",
                                    marker=dict(size=10),
                                    name=f"Diagnostic: {c_name}",
                                )
                            )
    ...
    with tab4:
        ...
        if src == "Local (outputs/)":
            ...
            if not paramiko:
                ...
                with st.expander("🔑 Jump Host Configuration", expanded=True):
                    ...
                    otp = c_otp.text_input(
                        "OTP Token", type="password", key="remote_otp"
                    )
                with st.expander("🎯 Target Server Configuration", expanded=True):
                    ...
                    pwd = c5.text_input(
                        "ECS Password", type="password", key="remote_pwd"
                    )
                    rp = st.text_input(
                        "Remote Path",
            ...
            uploaded_file = st.file_uploader(
                "Upload W&B Export CSV", type="csv", key="csv_uploader"
            )
            if uploaded_file:
                if st.button("📊 Run Statistical Analysis on Uploaded CSV"):
                    ...
                    if not df_res.empty:
                        st.session_state["leaderboard_df"] = df_res
                        st.rerun()
        elif src == "W&B API (Live)":
            w_c1, w_c2 = st.columns(2)
            w_entity = w_c1.text_input(
                "W&B Entity", value="victoria-university-of-wellington", key="wb_ent"
            )
            w_project = w_c2.text_input(
                "W&B Project", value="fishy-business", key="wb_proj"
            )
            w_key = st.text_input(
                "W&B API Key (Optional)", type="password", key="wb_key"
            )
            if st.button("🌐 Fetch & Analyze Live W&B Data"):
                with st.spinner("Talking to W&B..."):
        ...
        if not df_summary.empty:
            ...
            st.dataframe(
                df_summary.sort_values(
                    ["Dataset", "Test"], ascending=[True, False]
            ...
            if "raw_results_df" in st.session_state:
                render_advanced_benchmarks(
                    df_summary, st.session_state["raw_results_df"], color_map
                )
else:
