    ...
    for ds in expected_datasets:
        for model in expected_models:
            ...
            if count < expected_runs:
                ...
                print(f"❌ {ds.ljust(15)} | {model.ljust(12)} | Missing: {str(missing).rjust(2)} runs (Found {count})")
                missing_combinations.append((ds, model, missing))
            elif count > expected_runs:
                print(f"⚠️  {ds.ljust(15)} | {model.ljust(12)} | Extra:   {str(count-expected_runs).rjust(2)} runs (Found {count})")
