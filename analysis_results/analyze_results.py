...
def analyze():
    ...
    print(
        f"{'Dataset':<15} | {'Model':<12} | {'Mean Acc':<10} | {'Std Dev':<10} | {'P-Value':<10}"
    )
    print("-" * 65)
    ...
    for ds in datasets:
        ...
        acc_aug = ds_data[ds_data["model"] == "augformer"][
            "val_balanced_accuracy"
        ].values
        acc_gate = ds_data[ds_data["model"] == "gatedmlp"][
            "val_balanced_accuracy"
        ].values
        ...
        print(
            f"{ds:<15} | augformer    | {mean_aug:.4f}     | {std_aug:.4f}     | {p_val:.4f}"
        )
